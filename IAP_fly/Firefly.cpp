...
Firefly::Firefly(){} //constructor of the class firefly

/*
the "timerAndPinConfiguration"-function have to be called in the setup to configure the the Timer and the Outputs with the IR-LED's and IR-Receiver
*/

void Firefly::timerAndPinConfiguration()
{
	/*
	Initialize Timer1:
...
{
	epsilon = mapFloat(analogRead(A0), 0, 1023, -0.01, 2.5); //reading in the analog value on pin PC0/A0 and map that value to a value between 0.01 and 0.2
    if (epsilon < 0) {
        epsilon = 0;
    }
    frequency = mapFloat(analogRead(A1), 0, 1023, 0.5, 4.);
...
void Firefly::flashMirolloStrogatzModel()
{
	int start_flash = millis();
  	...
  	while((current_time - start_flash) <= VISIBLE_FLASH_LENGTH)
  	{
    	...
    	*/
        ...
        if ((current_time - start_flash) >= IR_FLASH_LENGTH + IR_FLASH_BLIND) {
            // still check for flashes
            flashReceiveCheck();
            receiveHandlerMirolloStrogatzModel();
            resetFlashReceive();
        }
    	current_time = millis();
  	}

    Serial.println("Coupling strength epsilon: ");
    Serial.println(epsilon);
    Serial.println("Base frequency: ");
    ...
    flash_not_yet_seen = true;
  	x = double(max(int(x*PREC)%PREC, MAX_RESET_X))/PREC;
    // Serial.print("MODULO GIVES: ");
    // Serial.println(x);
  	PHI_RAW = 0;
}
...
void Firefly::receiveHandlerMirolloStrogatzModel()
{
	//The IR-receiver has an internal pull-up resistor! So it's important to look for an LOW pin if a flash is received.
  	...
  	if(flash_received && flash_not_yet_seen)
  	{
        if (x<0.5) {
